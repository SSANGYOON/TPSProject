...
FFramePackage ULagCompensationComponent::GetFrameToCheck(ATPSCharacter* HitCharacter, float HitTime)
{
	...
	while (Older->GetValue().Time > HitTime) // older가 여전히 hitTime보다 빠른가?
	{
...
void ULagCompensationComponent::ServerScoreRequest_Implementation(ATPSCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation, float HitTime, AWeapon* DamageCauser)
{
...
void ULagCompensationComponent::ShotgunServerScoreRequest_Implementation(const TArray<ATPSCharacter*>& HitCharacters, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations, float HitTime)
{
